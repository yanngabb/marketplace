...
TODO: mix between custodial and non custodial
User + Regulator + Application should provide a share of the signature such that the user is still the owner of its asset but security is also ensure because the application work in a much more secure environment. Note that we need to find a way to prevent censorship.
...
Finally, our security analysis shows that our system fulfills its role in the fight against the black market and fraud well and our evaluation shows that it is possible to put NFTiX into production because the costs would not be too high and the performance sufficient.
