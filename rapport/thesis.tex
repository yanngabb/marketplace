...
Finally, the graph node is responsible for indexing the smart contract such that we can easily query data. The marketplace is responsible for listing the resale and swap offers. In the next section we give a more details description of each components.
...
This section describes the core of NFTiX which is responsible for enforcing the business logic. The core is made up of four interacting smart contracts, the Identity contract, the TIX contract, the Ticketing contract and the Exchange contract. For each contract we first describe the initial design and then the improvement that were made fixing the initial design mistakes. You can find several flow diagrams that should help understand how NFTiX works. In these diagrams the steps highlighted in red are the off-chain operations. In blue are the transactions sent to the blockchain. And finally in green are the on-chain operations.
...
We bring a small improvement to the ERC20 standard that we think is really useful to reduce the number of transaction required when TIX are exchange against another asset such as a ticket. This improvement consists of creating an off-chain proof allowing a smart contract to transfer some of their TIX without having to do a approval transaction first. We call this an externally approved transfer. This kind of approval is really useful to build the exchange. We will come back on its usage later.

Here is how it works. The user creates a TIX approval proof with the address of the approved contract, his address, the address of the receiver and the amount to transfer. The proof is then put as argument in a transaction and is checked by a contract. Note that this kind of approval must be used with caution. We give an analysis of what could go wrong if this mechanism is misused in the security chapter.
...
A spectator can transfer a ticket to another spectator. For a transfer to be executed successfully, the transaction initiator must be the ticket owner, the ticket recipient must be a registered viewer, the event status must be OPEN, and the event status must be ticket is VALID. You can find the flow diagram of the transfer in the figure \hyperref[fig:transfer]{\ref{fig:transfer}}.
...
\subsubsection{Improvements}
To pay the event registration and the minting fee, the organizer need to approve the Ticketing contract to transfer some of his TIX to NFTiX. It means that the organizer must first do a approval transaction in the TIX contract and then an event registration transaction in the Ticketing smart contract. Doing two transaction is not ideal because the organizer has to pay two times the base gas fee. However, if the organizer create an approval proof to authorize the Ticketing contract to make the TIX transfer on its behalf. With this approach, the organizer only need to do a single transaction and the event registration is now atomic.

As you can see this externally approved transfer are very useful. We extend the ERC721 standard with this same mechanism. Having this mechanism in the TIX contract and the Ticketing contract enable us to make atomic resale and swap. We details this in the next section.

We also want to mention an improvement that has finally not be kept. With the current design a single smart contract handle all the events and all the tickets for all the organizers. It could be useful to have a dedicated Ticketing contract per organizer in order to not mix data of different organizer together and to be able to have a specific on-chain logic per organizer. To do that we can use a Factory contract similar to the one of UniswapV3. When an organizer registers an event on the Factory, it deploys a Ticketing smart contract dedicated for this event. If we want a specific business logic we can manually deploy a Ticketing contract and then register it in the Factory contract. We finally do not keep this improvement because we want keep a generic and simple on-chain logic. For specific business logics we prefer to use oracles.

\subsection{Exchange smart contract}

\subsubsection{Initial design}
The Exchange smart contract allows the spectator to resell and swap their ticket in complete safety. The contract lists all resale and swap offers. In addition, this smart contract verifies that each operation has been approved by the Approver before executing it.
...
TODO: explain how and why we change the swap logic

You can find the flow diagram of the swapV2 in the figure \hyperref[fig:swapV2]{ \ref{fig:swapV2}}.
...
To go even further in the process, we could also aggregate the proofs into one in order to reduce the complexity of the verification of the proofs on the smart contract and therefore the transaction execution costs. For instance the approval proofs of the user and the Approver can be aggregated because they are both checked by the Exchange contract. The three TIX transfer approval can also be aggregate such that the TIX contract only need to verify a single proof. Of course, the TIX contract must also support batch of transfer. Finally, note that we need to construct proofs that have homomorphic properties in order to aggregate proofs.
